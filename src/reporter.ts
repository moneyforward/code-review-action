...
const debug = util.debuglog('@moneyforward/code-review-action/reporter');

export type ReportWriter = stream.Writable;
...
export class ReporterRepository {
  ...
  protected readonly repository: Map<string | undefined, ReporterConstructor>;

  constructor(protected readonly defaultReporterType: ReporterConstructor, ...othors: ReporterConstructor[]) {
    this.repository = new Map(othors.concat(defaultReporterType).map(constructor => [constructor.name, constructor]));
  }

  register(reporterType: ReporterConstructor, name?: string): void {
  ...
  get(notation?: string): ReporterConstructor {
    const parse = (notation?: string): ReporterConstructor[] => {
      return (notation || '').split(',')
        ...
        .map(name => this.repository.get(name) || this.defaultReporterType);
    };
